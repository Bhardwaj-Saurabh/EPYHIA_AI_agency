...
Produced by `eval/eval.py` on 2026-08-11 15:59 UTC against the running agency.
...
- **Agency:** https://epyhia-gateway.fly.dev

| Check | Area | Result | Points | Evidence |
|---|---|---|---|---|
| `order-persists` | The three deliverables are real | ✅ PASS | 10/10 | purchase 8f4d89d9-cdfb-5367-98e7-c499eacee95d: 5400p PAID persisted (webhook received=True); redelivery duplicate=true, still exactly 1 row |
| `no-duplicates` | The Action Gate | ✅ PASS | 5/5 | same run ad9da5f1-dc28-4db5-8a38-32342b0eb063 returned (replayed=true); deploy actions 3 and orders 7 unchanged |
...
| `approval-before-irreversible` | The Action Gate | ✅ PASS | 5/5 | 3 executed irreversible actions (['deploy']), every one carries approved_by + approved_at <= executed_at |
| `audit-cost` | The Action Gate | ✅ PASS | 5/5 | 23 audit rows, all mode=TEST with payload hash + idempotency key; run spend 1952659 of 2000000 microdollars ($1.95 of $2.00) |
...
| Ships & runs from clean clone | 10/10 |
| **Total (automated)** | **96/100** |
